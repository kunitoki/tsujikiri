...
# Emit one callable per arity for parameters that have C++ default values
...
Target languages that have no defaulted-argument concept cannot express
`f(int a, float b = 1.0f)` as a single callable — a binding generated from that
signature alone would require both arguments. When this key is `true`, every
callable with trailing defaulted parameters is emitted once per reachable
arity: the full signature plus one truncated entry per dropped trailing default,
in descending order.

Constructors only need the shortened signature list (`addConstructor<void(*)(int)>`
resolves the C++ defaults itself); methods and free functions additionally need a
forwarding lambda, so the extra entries carry `is_default_expansion: True` and an
`omitted_params` list in the template context.

A truncation whose signature already matches another declaration in the same group
is skipped, so `void f(int); void f(int, float = 1.0f);` does not produce a
duplicate one-argument binding.

Leaving the key unset (`None`) inherits the value from the format named by
`extends`; setting it explicitly to `false` overrides an inherited `true`.

| Format | `expand_default_arguments` | Defaults expressed as |
|--------|---------------------------|-----------------------|
| `luabridge3` | `true` | one `addFunction` entry per arity |
| `luals` | `true` | `---@overload fun(...)` per arity |
| `pybind11` | unset | `py::arg("x") = 1.0` |
| `pyi` | unset | `x: float = 1.0` |
...
Unary and binary operators of the same spelling are discriminated by arity and
bind to **different** metamethods, so a class declaring both `operator-()` and
`operator-(const T&)` gets `__unm` and `__sub` respectively. When the C++ spelling
is overloaded the member pointer is disambiguated with a `luabridge::overload<>`
cast.
...
.beginClass<ns::Vec3>("Vec3")
    ...
    .addFunction("__unm", luabridge::overload<>(&ns::Vec3::operator-))
    .addFunction("__sub", luabridge::overload<double>(&ns::Vec3::operator-))
...
parameter, rather than as a module-level function. The operand class is resolved
by qualified name, falling back to the free function's own namespace for an
unqualified spelling. Free operators also follow the arity convention: their
left-hand operand is an extra leading parameter, so `operator-(const Vec3&)` is
unary and `operator-(const Vec3&, const Vec3&)` is binary.
...
// Generated binding, inside the Vec3 class block:
.addFunction("__add", &ns::operator+)
```

A free operator that the format does not map, or whose first parameter names no
bound class, is left in the module-level function groups unchanged — this is why
`operator<<` (whose first parameter is `std::ostream&`) never attaches.
...
| `class_overloaded_metamethod` | Single overloaded operator metamethod cast |
| `class_free_operators` | All free-operator metamethod bindings for the class |
| `class_free_operator_group` | One free-operator group (one metamethod) |
| `class_free_operator_overloaded` | Single overloaded free-operator entry |
...
Unary and binary operators of the same spelling annotate distinct metamethods
(`__unm` vs `__sub`), matching the luabridge3 bindings. Free-function operators
are annotated on their first operand's class via the `class_free_operators` block,
with the leading operand dropped because it becomes `self`.

Defaulted arguments are annotated as extra `---@overload fun(...)` lines rather
than optional parameters, so the annotations mirror the one-callable-per-arity
bindings exactly:
...
---@param scale number
---@return Vec2
---@overload fun(self: Vec2, dx: number, dy: number): Vec2
---@overload fun(self: Vec2, dx: number): Vec2
function Vec2:offset(dx, dy, scale) end
```
...
| `class_free_operators` | All free-operator dunder bindings for the class |
| `class_free_operator_group` | One free-operator group (one dunder) |
| `class_free_operator` | Single free-operator `.def(...)` call |
