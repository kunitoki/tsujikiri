...
def _module_with_classes(*names: str, source_file: str | None = None) -> TIRModule:
    classes = [TIRClass(name=n, qualified_name=f"ns::{n}", namespace="ns", source_file=source_file) for n in names]
    return TIRModule(
        name="m",
        classes=classes,  # type: ignore[arg-type, list-item]
        class_by_name={c.name: c for c in classes},
    )  # type: ignore[misc]


def _module_with_methods(*method_names: str) -> tuple[TIRModule, TIRClass]:
    methods = [TIRMethod(name=n, spelling=n, qualified_name=f"Cls::{n}", return_type="void") for n in method_names]
...
class TestClassBlacklist:
    def test_exact_match_suppresses(self):
        ...
        FilterEngine(FilterConfig(classes=ClassFilter(blacklist=[FilterPattern("Bar")]))).apply(mod)
    ...
    def test_regex_match_suppresses(self):
        ...
        FilterEngine(FilterConfig(classes=ClassFilter(blacklist=[FilterPattern(".*Impl$", is_regex=True)]))).apply(mod)
        assert mod.classes[0].emit is False
        assert mod.classes[1].emit is True

    def test_no_match_keeps_all(self):
        mod = _module_with_classes("Foo", "Bar")
        FilterEngine(FilterConfig(classes=ClassFilter(blacklist=[FilterPattern("Baz")]))).apply(mod)
...
class TestClassWhitelist:
    ...
    def test_non_empty_whitelist_suppresses_others(self):
        ...
        FilterEngine(FilterConfig(classes=ClassFilter(whitelist=[FilterPattern("Foo")]))).apply(mod)
        emitted = [c.name for c in mod.classes if c.emit]
        assert emitted == ["Foo"]

    def test_regex_whitelist(self):
        mod = _module_with_classes("MyFoo", "MyBar", "Other")
        FilterEngine(FilterConfig(classes=ClassFilter(whitelist=[FilterPattern("My.*", is_regex=True)]))).apply(mod)
        emitted = {c.name for c in mod.classes if c.emit}
...
class TestClassInternal:
    def test_internal_suppresses(self):
        ...
        FilterEngine(FilterConfig(classes=ClassFilter(internal=[FilterPattern("BaseHelper")]))).apply(mod)
        assert mod.classes[0].emit is True
        assert mod.classes[1].emit is False
...
class TestMethodFilter:
    def test_global_blacklist(self):
        ...
        FilterEngine(FilterConfig(methods=MethodFilter(global_blacklist=[FilterPattern("remove")]))).apply(mod)
    ...
    def test_global_blacklist_regex(self):
        ...
        FilterEngine(
            FilterConfig(methods=MethodFilter(global_blacklist=[FilterPattern("operator.*", is_regex=True)]))
        ).apply(mod)
        emitted = [m.name for m in cls.methods if m.emit]
    ...
    def test_per_class_does_not_affect_other_classes(self):
        methods = [
            TIRMethod(
                name="perClassOnly", spelling="perClassOnly", qualified_name="Other::perClassOnly", return_type="void"
            )
        ]
...
class TestConstructorFilter:
    def _cls_with_ctors(self, *param_lists):
        ctors = [TIRConstructor(parameters=[TIRParameter("x", t) for t in pl]) for pl in param_lists]
        cls = TIRClass(name="C", qualified_name="ns::C", namespace="ns", constructors=ctors)  # type: ignore[arg-type]
    ...
    def test_include_false_suppresses_all(self):
        ...
        FilterEngine(FilterConfig(constructors=ConstructorFilter(include=False))).apply(mod)
        assert all(not c.emit for c in cls.constructors)

    def test_include_true_keeps_all(self):
        mod, cls = self._cls_with_ctors([], ["int"])
        FilterEngine(FilterConfig(constructors=ConstructorFilter(include=True))).apply(mod)
        assert all(c.emit for c in cls.constructors)

    def test_signature_filter(self):
        mod, cls = self._cls_with_ctors([], ["int"])
        FilterEngine(
            FilterConfig(constructors=ConstructorFilter(include=True, signatures=[FilterPattern("int")]))
        ).apply(mod)
...
class TestFieldFilter:
    def _cls_with_fields(self, *names):
        ...
        cls = TIRClass(name="C", qualified_name="ns::C", namespace="ns", fields=list(fields))  # type: ignore[arg-type]
        mod = TIRModule(name="m", classes=[cls], class_by_name={"C": cls})  # type: ignore[arg-type, list-item]
        return mod, cls

    def test_global_blacklist(self):
        mod, cls = self._cls_with_fields("keep_", "pimpl_")
        FilterEngine(FilterConfig(fields=FieldFilter(global_blacklist=[FilterPattern("pimpl_")]))).apply(mod)
        emitted = [f.name for f in cls.fields if f.emit]
        assert emitted == ["keep_"]

    def test_per_class_blacklist(self):
        mod, cls = self._cls_with_fields("keep_", "secret_")
        FilterEngine(FilterConfig(fields=FieldFilter(per_class={"C": [FilterPattern("secret_")]}))).apply(mod)
        emitted = [f.name for f in cls.fields if f.emit]
        assert emitted == ["keep_"]
...
class TestFunctionFilter:
    def _mod_with_fns(*names):
        fns = [TIRFunction(name=n, qualified_name=f"ns::{n}", namespace="ns", return_type="void") for n in names]
        return TIRModule(name="m", functions=list(fns))  # type: ignore[arg-type, list-item]

    def test_blacklist(self):
        mod = TIRModule(
            name="m",
            functions=[  # type: ignore[arg-type, list-item]
        ...
        FilterEngine(FilterConfig(functions=FunctionFilter(blacklist=[FilterPattern("skip")]))).apply(mod)
    ...
    def test_whitelist(self):
        ...
        FilterEngine(FilterConfig(functions=FunctionFilter(whitelist=[FilterPattern("foo")]))).apply(mod)
        emitted = [f.name for f in mod.functions if f.emit]
        assert emitted == ["foo"]
...
class TestEnumFilter:
    ...
    def test_blacklist(self):
        mod = TIRModule(
            name="m",
            enums=[  # type: ignore[arg-type, list-item]
                TIREnum(name="Keep", qualified_name="ns::Keep"),
                TIREnum(name="Skip", qualified_name="ns::Skip"),
            ],
        )
        FilterEngine(FilterConfig(enums=EnumFilter(blacklist=[FilterPattern("Skip")]))).apply(mod)
    ...
    def test_whitelist_suppresses_non_matching(self):
        ...
        FilterEngine(FilterConfig(enums=EnumFilter(whitelist=[FilterPattern("Keep")]))).apply(mod)
        emitted = [e.name for e in mod.enums if e.emit]
        assert emitted == ["Keep"]
...
class TestPreSuppressedNodes:
    ...
    def test_pre_suppressed_method_skips_filter(self):
        ...
        method.emit = False
        cls = TIRClass(name="C", qualified_name="ns::C", namespace="ns", methods=[method])  # type: ignore[arg-type]
        mod = TIRModule(name="m", classes=[cls], class_by_name={"C": cls})  # type: ignore[arg-type, list-item]
        FilterEngine(FilterConfig()).apply(mod)
    ...
    def test_pre_suppressed_field_skips_filter(self):
        ...
        f.emit = False
        cls = TIRClass(name="C", qualified_name="ns::C", namespace="ns", fields=[f])  # type: ignore[arg-type]
        mod = TIRModule(name="m", classes=[cls], class_by_name={"C": cls})  # type: ignore[arg-type, list-item]
        FilterEngine(FilterConfig()).apply(mod)
...
class TestVarargsFilter:
    def test_varargs_method_suppressed(self):
        ...
        normal_m = TIRMethod(
            name="info",
            spelling="info",
            qualified_name="ns::Foo::info",
            return_type="void",
            is_varargs=False,
        )
        cls = TIRClass(name="Foo", qualified_name="ns::Foo", namespace="ns", methods=[varargs_m, normal_m])  # type: ignore[arg-type]
    ...
    def test_non_varargs_not_affected(self):
        ...
        cls = TIRClass(name="Foo", qualified_name="ns::Foo", namespace="ns", methods=[m])  # type: ignore[arg-type]
        mod = TIRModule(name="m", classes=[cls], class_by_name={"Foo": cls})  # type: ignore[arg-type, list-item]
        FilterEngine(FilterConfig()).apply(mod)
...
class TestBaseFilter:
    def test_base_outside_namespace_suppressed(self) -> None:
        ...
        cls = TIRClass(name="MyError", qualified_name="ns::MyError", namespace="ns", bases=[base])  # type: ignore[arg-type]
    ...
    def test_base_inside_namespace_kept(self) -> None:
        ...
        cls = TIRClass(name="Child", qualified_name="ns::Child", namespace="ns", bases=[base])  # type: ignore[arg-type]
